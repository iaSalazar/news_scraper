...
def _extract():
	...
	for news_site_uid in news_sites_uids:
		...
		subprocess.run(['find','-name','{}*'.format(news_site_uid), 'exec', 'mv','{}_.csv'.format(news_site_uid), ';'], cwd='./extract')
...
def _transform():
	...
	for news_site_uid in news_sites_uids:
		...
		subprocess.run(['python', 'newspaper_recipie.py', dirty_data_filename], cwd = './transform')
...
def _load():
	logger.info('Starting load proces')
	for news_site_uid in news_sites_uids:
	...
	logger.info('finished load proces')
